...
export async function GET() {
  return NextResponse.json({ missions: distributionStore.listMissions() });
}
...
export async function POST(request: Request) {
  try {
    ...
    const input: MissionInput = {
      goal: body.goal.trim(),
      market: body.market?.trim() || "Global",
      language: body.language === "ru" ? "ru" : "en",
      autonomy: body.autonomy === "auto" || body.autonomy === "draft" ? body.autonomy : "approve"
    };
    const plan = await buildDistributionPlan(input);
