...
export interface DashboardSnapshot { missions:MissionRecord[]; actions:DistributionActionRecord[]; content:ContentDraft[]; leads:Lead[]; products:ProductRecord[]; outreach:OutreachSequence[]; replies:InboundReply[]; stats:{activeMissions:number;queuedActions:number;approvalsNeeded:number;completedActions:number;researchedLeads:number;products:number;outreachPrepared:number;inboundReplies:number;positiveReplies:number}; }
