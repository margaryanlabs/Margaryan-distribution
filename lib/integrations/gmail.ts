...
export async function markGmailRead(messageId:string){const token=await resolveAccessToken();const res=await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${encodeURIComponent(messageId)}/modify`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({removeLabelIds:["UNREAD"]})});if(!res.ok)throw new Error(`Gmail mark-read failed: ${res.status} ${await res.text()}`);return res.json();}
