...
import {sendGmailMessage} from "./gmail"; import {publishXPost} from "./x"; import {publishLinkedInPost} from "./linkedin"; import {publishInstagramImage} from "./instagram"; import {startOutboundCall} from "./twilio";
...
export async function executeProviderAction(action:ExecuteRequest){
  if(action.channel==="email"&&action.kind==="send_email")return sendGmailMessage({to:String(action.payload.to||""),subject:String(action.payload.subject||""),body:String(action.payload.body||"")});
