...
export async function runDueAutoActions(limit=10){
  ...
  for(const action of due){
    ...
    try{const result=await executeProviderAction(request);distributionStore.updateAction(action.recordId,{status:"succeeded",result,executedAt:new Date().toISOString(),error:undefined});results.push({id:action.recordId,status:"succeeded",channel:action.channel,simulated:typeof result==="object"&&result!==null&&"simulated" in result});}
    catch(error){const message=error instanceof Error?error.message:"unknown error";distributionStore.updateAction(action.recordId,{status:"failed",error:message,retryCount:action.retryCount+1});results.push({id:action.recordId,status:"failed",error:message});}
  }
