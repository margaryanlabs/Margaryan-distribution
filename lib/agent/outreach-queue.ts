...
export async function prepareLeadOutreach(mission:MissionRecord,lead:Lead,product?:ProductRecord){
  ...
  const actions:PlannedAction[]=[
    ...
    {id:`voice-${lead.id}`,channel:"voice",kind:"call",objective:`Qualify ${lead.company}`,rationale:voiceQuality.pass?"Voice waits for a successful first touch and remains blocked until jurisdiction, DNC and permitted calling hours are verified":`Voice copy quality gate failed (${voiceQuality.score}/100)`,mode:"BLOCKED",scheduledOffsetHours:24,payload:{leadId:lead.id,to:lead.phone||"",leadName:lead.company,objective:draft.callOpening,sequenceType:"cold",sequenceStep:2,dependsOnActionId:initialActionId,jurisdictionVerified:false,withinAllowedHours:false,...qualityPayload(voiceQuality)}}
  ];
