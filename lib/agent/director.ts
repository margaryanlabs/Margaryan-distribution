...
import type { Language, Lead } from "@/lib/types";
...
export async function runAutopilot(options:AutopilotOptions={}){
  const leadTarget=Math.max(1,Math.min(100,Math.round(options.leadTarget||25)));const researchPerTick=Math.max(1,Math.min(10,Math.round(options.researchPerTick||10)));const outreachPerTick=Math.max(1,Math.min(20,Math.round(options.outreachPerTick||5)));const smmDays=Math.max(3,Math.min(14,Math.round(options.smmDays||7)));const startedAt=new Date().toISOString();
  ...
  for(const mission of missions){
    ...
    let leads=distributionStore.listLeads().filter(lead=>lead.missionId===mission.id);let reachable=leads.filter(isReachable);
    const researchCeiling=leadTarget*3;
    if(reachable.length<leadTarget&&leads.length<researchCeiling){
      if(process.env.OPENAI_API_KEY){try{const requested=Math.min(researchPerTick,leadTarget-reachable.length);const candidates=await researchBusinessLeads(mission,requested,product);const added=distributionStore.addLeads(candidates.map(candidate=>({...candidate,missionId:mission.id,language:mission.input.language as Language,stage:"researched" as const})));steps.push({stage:"research",requested,researched:candidates.length,added:added.length});leads=distributionStore.listLeads().filter(lead=>lead.missionId===mission.id);reachable=leads.filter(isReachable);}catch(error){steps.push({stage:"research",error:error instanceof Error?error.message:"Lead research failed"});}}
      else steps.push({stage:"research",skipped:"OPENAI_API_KEY is not configured"});
    }else steps.push({stage:"research",skipped:reachable.length>=leadTarget?"Reachable lead target already reached":"Research ceiling reached; review ICP or evidence quality",count:leads.length,reachable:reachable.length});
    const existingSequences=new Set(distributionStore.listOutreach().filter(item=>item.missionId===mission.id).map(item=>item.leadId));const outreachCandidates=leads.filter(lead=>!existingSequences.has(lead.id)&&!lead.optedOut&&lead.stage!=="do_not_contact"&&(lead.score||0)>=55&&isReachable(lead)).sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,outreachPerTick);let prepared=0;
    ...
    missionReport.leads=leads.length;missionReport.reachableLeads=reachable.length;missionReport.outreachPrepared=distributionStore.listOutreach().filter(item=>item.missionId===mission.id).length;missionReport.contentAssets=distributionStore.listContent().filter(item=>item.missionId===mission.id).length;report.missions.push(missionReport);
  }
  if(options.runActions!==false){try{report.execution=await runDueAutoActions(40);}catch(error){report.execution={error:error instanceof Error?error.message:"Action runner failed"};}}
