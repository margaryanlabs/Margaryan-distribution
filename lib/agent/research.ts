...
export async function researchBusinessLeads(mission: MissionRecord, limit = 8, product?: ProductRecord): Promise<LeadCandidate[]> {
  ...
  const response = await client.responses.create({
    model: process.env.OPENAI_RESEARCH_MODEL || "gpt-5.6-terra", reasoning: { effort: "medium" }, tools: [{ type: "web_search_preview", search_context_size: "medium" }],
    instructions: [
      "You are the research layer of a B2B distribution system.",
      "Find real companies that plausibly match the mission using current public web information and the supplied product brain.",
  ...
  return parsed.slice(0, Math.min(limit, 10)).filter((lead) => typeof lead.company === "string" && lead.company.trim()).map((lead) => ({
    company: lead.company.trim(), website: typeof lead.website === "string" ? lead.website.trim() : undefined, country: typeof lead.country === "string" ? lead.country.trim() : mission.input.market,
    fitReason: typeof lead.fitReason === "string" ? lead.fitReason.trim() : "", score: Math.max(0, Math.min(100, Number(lead.score || 0))), research: typeof lead.research === "string" ? lead.research.trim() : ""
  }));
}
